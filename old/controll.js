...
let GIF_TEMP = [];
let GIF_TEMP_REAL = [];
let gif_for_show = null;
let gif_for_real = null;
...
function render_GIF() {
  gif_for_show = new GIF({
    workers: 2,
    quality: 10
  });
  gif_for_real = new GIF({
    workers: 2,
    quality: 10
  });
  GIF_TEMP.forEach((el) => {
    gif_for_show.addFrame(el);
  });
  GIF_TEMP_REAL.forEach((el) => {
    gif_for_real.addFrame(el);
  });

  gif_for_show.on('finished', function (blob) {
    IMAGE_URL = URL.createObjectURL(blob);

    RENDER_PICTURE.style.backgroundImage = `url(${IMAGE_URL})`;
    currentResult.forShow = IMAGE_URL;
    NO_LOADING();
    // showTools();
    showResult();
  });
  ...
  gif_for_real.on('finished', function (blob) {
    currentResult.forReal = URL.createObjectURL(blob);
  });
